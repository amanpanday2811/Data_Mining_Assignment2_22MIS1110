...
clustering_data <- data[, c("Age", "Credit_Score")]

# Perform CLARA clustering with different sampling sizes
set.seed(123)  # For reproducibility
sampling_sizes <- c(3, 5, 10)  # Change as desired
results <- lapply(sampling_sizes, function(size) {
  clara_result <- clara(clustering_data, k = 3, sample = size)
  return(clara_result)
})

# Print clustering results
for (i in seq_along(sampling_sizes)) {
  cat("Sampling size:", sampling_sizes[i], "\n")
  print(results[[i]]$clustering)
  cat("\n")
}
